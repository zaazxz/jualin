...
import { Mail, ArrowRight, Sparkles, CheckCircle2, AlertCircle, ArrowLeft, Moon, Sun, Globe } from 'lucide-react';
            ...
            <div className="w-full max-w-[440px] animate-in fade-in zoom-in duration-500">
                ...
                <div className="w-full bg-jual-card backdrop-blur-xl rounded-2xl border border-jual-border p-6 sm:p-10 shadow-xl relative overflow-hidden group/card">
                    ...
                    <div className="mb-6 text-center sm:text-left">
                        <h2 className="text-xl font-semibold text-jual-text-main tracking-tight">{t('forgot_password_title')}</h2>
                    ...
                    <form className="space-y-6" onSubmit={submit}>
                        ...
                        <button
                            type="submit"
                            disabled={processing}
                            className="w-full bg-emerald-500 hover:bg-emerald-400 text-slate-900 font-bold py-3.5 rounded-xl transition-all duration-300 flex items-center justify-center gap-2 shadow-[0_4px_20px_rgba(16,185,129,0.2)]"
                        >
                            <span>{processing ? t('processing') : t('send_reset_link')}</span>
                            {!processing && <ArrowRight className="w-4 h-4" />}
                        </button>
                    </form>

                    {/* Back to Login */}
                    <div className="mt-8 text-center border-t border-jual-border pt-6">
                        <Link href={route('login')} className="inline-flex items-center gap-2 text-sm text-jual-text-muted hover:text-emerald-500 transition-colors font-medium">
